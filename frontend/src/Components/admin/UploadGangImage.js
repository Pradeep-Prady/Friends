...
import { createGangImage } from "../../actions/gangImageActions";
...
export default function UploadGangImage() {
  const [image, setImage] = useState("");
  const [imagePreview, setImagePreview] = useState(
    "/images/default_avatar.png"
  );

  const { isGangImageCreated, loading, error } = useSelector(
  ...
  const submitHandler = (e) => {
    ...
    dispatch(createGangImage(formData));
  };

  useEffect(() => {
    if (isGangImageCreated) {
      toast.success("Gang Image Posted", {
  ...
  return (
    <>
      <MetaData title={"Upload Gang Image"} />
      <div
